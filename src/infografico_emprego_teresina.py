...
class ProcessadorRAIS:
    ...
    def __init__(self, data_dir: Path):
        ...
        self.shapefile_path = data_dir / "bairros" / "BAIRROS_2013.shp"
        self.sdu_shapefile_path = data_dir / "sdu" / "Divisao SDUs.shp"
    ...
    def carregar_shapefile_sdu(self) -> gpd.GeoDataFrame:
        """Carrega e padroniza shapefile de zonas SDU"""
        shp = gpd.read_file(self.sdu_shapefile_path)
        col_sdu = None
        for col in shp.columns:
            if any(term in col.upper() for term in ['SDU', 'ZONA', 'NOME', 'NM_', 'REGIA']):
                col_sdu = col
                break
        if col_sdu is None:
            raise ValueError("Coluna de SDU não encontrada no shapefile de SDU")
        shp = shp[[col_sdu, 'geometry']].copy()
        shp.columns = ['sdu', 'geometry']
        shp['sdu'] = shp['sdu'].astype(str).str.strip().str.upper()
        return shp
    
    def atribuir_sdu(self, df: pd.DataFrame, shp_sdu: gpd.GeoDataFrame) -> pd.DataFrame:
        """Atribui cada CEP à sua zona SDU correspondente"""
        print("\n  Atribuindo CEPs às zonas SDU...")
        geometry = [Point(lon, lat) for lon, lat in zip(df['longitude'], df['latitude'])]
        pontos_gdf = gpd.GeoDataFrame(df, geometry=geometry, crs='EPSG:4326')
        pontos_gdf = pontos_gdf.to_crs(shp_sdu.crs)
        resultado = gpd.sjoin(pontos_gdf, shp_sdu, how='left', predicate='within')
        atribuidos = resultado['sdu'].notna().sum()
        total = len(resultado)
        print(f"   ✓ CEPs atribuídos: {atribuidos:,}/{total:,} ({atribuidos/total*100:.1f}%)")
        resultado = resultado.dropna(subset=['sdu'])
        resultado = resultado.rename(columns={'sdu': 'sdu_nome'})
        return resultado
    ...
    def agregar_dados_sdu(self, df: pd.DataFrame) -> pd.DataFrame:
        """Agrega dados por zona SDU e ano"""
        print("\n Agregando dados (SDU)...")
        resultado = df.groupby(['sdu_nome', 'ano']).agg({
            'quantidade_vinculos_ativos': 'sum',
            'latitude': 'mean',
            'longitude': 'mean'
        }).reset_index()
    ...
    def gerar_mapa_com_grafico_sdu(self, df: pd.DataFrame, df_agg: pd.DataFrame, shp_sdu: gpd.GeoDataFrame, ano: int, output_path: Path):
        """Gera mapa por zonas SDU com gráfico de barras integrado"""
        print(f"\n  Gerando mapa SDU com gráfico para {ano}...")
        dados_mapa = shp_sdu.merge(
            df_agg[df_agg['ano'] == ano][['sdu_nome', 'quantidade_vinculos_ativos', 'percentual']],
            left_on='sdu', right_on='sdu_nome', how='left'
        )
        dados_mapa['quantidade_vinculos_ativos'] = dados_mapa['quantidade_vinculos_ativos'].fillna(0)
        fig = plt.figure(figsize=(16, 12), dpi=300)
        ax_mapa = plt.axes([0.05, 0.05, 0.9, 0.9])
        ...
        for spine in ax_mapa.spines.values():
            spine.set_visible(False)
        pontos_gdf = self.criar_pontos_visualizacao_ceps(df, shp_sdu, ano)
        if len(pontos_gdf) > 0:
            for cep in pontos_gdf['cep'].unique():
                pontos_cep = pontos_gdf[pontos_gdf['cep'] == cep]
                tamanho = pontos_cep.iloc[0]['tamanho']
        ...
        try:
            ...
            if len(dados_grafico) > 0 and dados_grafico['percentual'].sum() > 0:
                ax_grafico = plt.axes([0.08, 0.08, 0.28, 0.28])
                bars = ax_grafico.barh(dados_grafico['sdu_nome'].str.title(),
                                      dados_grafico['percentual'],
                                      color=CONFIG["cores"]["pontos"],
                                      alpha=0.85, edgecolor=CONFIG["cores"]["bordas"], linewidth=0.6)
                for i, (bar, valor) in enumerate(zip(bars, dados_grafico['percentual'])):
                ...
                ax_grafico.set_xticks([])
                ax_grafico.set_yticklabels([nome[:12] + '...' if len(nome) > 12 else nome 
                                           for nome in dados_grafico['sdu_nome'].str.title()],
                                           fontsize=CONFIG["fonte"]["legenda"] - 2,
                                           color=CONFIG["cores"]["texto"])
                ax_grafico.text(0.5, 1.05, f'Maiores zonas SDU', 
                               transform=ax_grafico.transAxes,
                               fontsize=CONFIG["fonte"]["legenda"],
                               color=CONFIG["cores"]["texto"],
                               weight='bold',
        ...
        for _, row in top5.iterrows():
            nome = row['sdu_nome']
            sdu_data = dados_mapa[dados_mapa['sdu'] == nome]
            if not sdu_data.empty and sdu_data['quantidade_vinculos_ativos'].iloc[0] > 0:
                centroid = sdu_data.geometry.centroid.iloc[0]
                label_text = f"{nome.title()}"
                ax_mapa.text(
                    centroid.x, centroid.y,
                    label_text,
                    fontsize=CONFIG["fonte"]["label"],
        ...
        ax_mapa.set_axis_off()
        ax_mapa.set_aspect('equal')
        titulo = f"Onde estavam os empregos formais por Zonas SDU em Teresina em {ano}?"
        top_zonas = df_agg[df_agg['ano'] == ano].nlargest(3, 'quantidade_vinculos_ativos')
        subtitulo = f"{', '.join(top_zonas['sdu_nome'].str.title().tolist())} eram as zonas com mais empregos formais"
        fig.text(0.05, 0.95, titulo, 
                fontsize=CONFIG["fonte"]["titulo"], 
                color=CONFIG["cores"]["texto"],
                weight='bold',
    ...
    def gerar_relatorio_sdu(self, df_agg: pd.DataFrame, output_path: Path):
        """Gera relatório estatístico por zonas SDU"""
        print("\n Gerando relatório (SDU)...")
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write("RELATÓRIO DE EMPREGO FORMAL - ZONAS SDU (TERESINA/PI)\n")
            f.write("=" * 50 + "\n\n")
            for ano in sorted(df_agg['ano'].unique()):
                ...
                f.write(f"Número de zonas SDU: {len(dados_ano)}\n")
                f.write(f"Média por zona: {dados_ano['quantidade_vinculos_ativos'].mean():.0f}\n\n")
                top5 = dados_ano.nlargest(5, 'quantidade_vinculos_ativos')
                f.write("Top 5 zonas SDU:\n")
                for i, (_, row) in enumerate(top5.iterrows(), 1):
                    f.write(f"  {i}. {row['sdu_nome'].title()}: {row['quantidade_vinculos_ativos']:,} ({row['percentual']:.1f}%)\n")
                f.write("\n")
    ...
    def executar_sdu(self):
        """Executa processamento para zonas SDU e gera materiais"""
        print("\nIniciando processamento para zonas SDU...")
        df = pd.read_csv(self.dados_path)
        anos_disponiveis = sorted(df['ano'].unique())
        global CONFIG
        CONFIG["anos_analise"] = anos_disponiveis
        shp_sdu = self.carregar_shapefile_sdu()
        print(f"   ✓ Shapefile SDU: {len(shp_sdu)} zonas")
        df_com_sdu = self.atribuir_sdu(df, shp_sdu)
        df_agg_sdu = self.agregar_dados_sdu(df_com_sdu)
        output_dir = self.data_dir.parent / "output"
        output_dir.mkdir(exist_ok=True)
        for ano in CONFIG["anos_analise"]:
            mapa_path = output_dir / f"mapa_emprego_teresina_sdu_{ano}.png"
            self.gerar_mapa_com_grafico_sdu(df, df_agg_sdu, shp_sdu, ano, mapa_path)
        relatorio_path = output_dir / "relatorio_emprego_teresina_sdu.txt"
        self.gerar_relatorio_sdu(df_agg_sdu, relatorio_path)
        print("\n" + "=" * 50)
        print("Processamento SDU concluído com sucesso!")
        print(f"Arquivos salvos em: {output_dir}")
...
def main():
    ...
    arquivos_necessarios = [
        data_dir / "bairros" / "BAIRROS_2013.shp",
    ...
    if (data_dir / "sdu" / "Divisao SDUs.shp").exists():
        processador.executar_sdu()
